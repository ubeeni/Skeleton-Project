<template>
  <div class="body">
    ...
    <div class="actions">
      <BtnLg text="추가" v-if="추가페이지" @click="addTransaction" color="var(--color-primary)" />
      <BtnLg
        text="수정"
        v-else-if="상세페이지"
      ...
      />
      <BtnLg text="확인" v-else @click="addTransaction" color="var(--color-primary)" />

      <BtnLg text="취소" @click="cancleTransaction" color="var(--color-semidark)" />

      <BtnLg text="삭제" v-if="true" @click="addTransaction" color="var(--color-semidark)" />
    </div>
  </div>
</template>
...
const transactionId = ref('JTF7') // 거래내역 ID - 현재는 하드코딩이지만 거래내역 화면에서 가져오는 값
